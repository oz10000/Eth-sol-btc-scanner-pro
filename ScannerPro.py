...
LOOKBACK_DAYS = 60                # Aumentado para tener más velas
SCAN_LOOKBACK = 30                 # Días para escáner (suficiente para últimas señales)
MC_ITER = 200
DATA_CACHE = {}
ATR_CACHE = {}
PIDELTA_CACHE = {}
...
MIN_FUTURE_VELAS = 50              # Velas necesarias hacia adelante para backtest
...
def scan_symbol_tf(symbol, tf):
    """Genera señales para un símbolo/TF, asegurando futuro suficiente."""
    ...
    for dt in high_tension_points:
        try:
            ...
            continue
        # Asegurar que hay suficientes velas hacia adelante
        if idx + MIN_FUTURE_VELAS >= len(price):
            continue
        # Para cada k, evaluar retorno futuro
...
# ============================================================
# BACKTEST DE UNA SEÑAL INDIVIDUAL (max_lookahead reducido)
# ============================================================
...
# ============================================================
# OPTIMIZACIÓN DE PARÁMETROS (con impresión reducida)
# ============================================================
def optimize_parameters(signals_df):
    ...
    for idx, combo in enumerate(combinations):
        ...
        for _, sig in filtered.iterrows():
            # Verificar futuro suficiente (ya debería cumplirse por el escáner, pero doble check)
            ...
            try:
                ...
                fallos += 1
                continue
            if idx_s + 30 >= len(df):  # max_lookahead=30
                fallos += 1
                continue
        ...
        n_trades = len(trades)
        # Imprimir solo cada 100 combinaciones para no saturar
        if idx % 100 == 0:
...
def analyze_top_signals(signals_df, best_params):
    ...
    print("\n" + "="*70)
    print("📊 TOP 3 SEÑALES LONG Y TOP 3 SHORT")
    print("="*70)
    for _, sig in top_signals.iterrows():
        print(f"\n🔹 {sig['Symbol']} {sig['TF']} {sig['Direction']} | Score: {sig['score']:.6f} | Tensión: {sig['tension']:.4f}")
    ...
    for _, sig in top_signals.iterrows():
        ...
        if res:
            results.append({
                ...
                'TF': sig['TF'],
                'Direction': sig['Direction'],
                'entry_time': sig['open_time'],
                'exit_type': res['exit_type'],
                'return': res['return'],
                'bars': res['bars'],
                'score_original': sig['score']
            })
        else:
            results.append({
                'Symbol': sig['Symbol'],
    ...
    print("\n📈 RESULTADOS INDIVIDUALES DE BACKTEST")
